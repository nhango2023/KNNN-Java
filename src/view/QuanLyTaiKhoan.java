...
public class QuanLyTaiKhoan extends JFrame {
	...
	public QuanLyTaiKhoan (String loaiTaiKhoan, String maNguoiSoHuu) {
		init();
		if (loaiTaiKhoan.equals("LTK002")) {
			for (int i = 0; i < tbTaiKhoan.getRowCount(); i++) {
                if (tbTaiKhoan.getValueAt(i, 3).equals(maNguoiSoHuu)) {
                	Object maTaiKhoan = tbTaiKhoan.getValueAt(i, 0);
                	if (maTaiKhoan==null) {
                		JOptionPane.showMessageDialog(null, "Chưa có tài khoản, vui lòng thêm tài khoản", "Warning", JOptionPane.WARNING_MESSAGE);
                	}
                    // Select the row           	
